...
const SLIPPAGE = new Percent(15, 100);
...
export abstract class BaseRoutingIntegTest {
  protected alice: JsonRpcSigner;
  protected isTenderlyEnvironmentSet: boolean;
  protected isTesterPKEnvironmentSet: boolean;

  abstract quote(): Promise<SwapRoute>;

  constructor(private envVars: EnvVars) {
    jest.retryTimes(0);
    // Flag for enabling logs for debugging integ tests
    if (this.envVars.INTEG_TEST_DEBUG) {
      setGlobalLogger(
        bunyan.createLogger({
  ...
  private executeSwap = async (
    ...
    if (swapType == SwapType.UNIVERSAL_ROUTER) {
      ...
      if (!permit) {
        const aliceP2 = Permit2__factory.connect(PERMIT2_ADDRESS, this.alice);
      ...
      tokenOutBefore = await hardhat.getBalance(this.alice._address, tokenOut);

      const transaction = {
        data: methodParameters.calldata,
        to: methodParameters.to,
        value: BigNumber.from(methodParameters.value),
        from: this.alice._address,
        gasPrice: BigNumber.from(2000000000000),
        type: 1,
      };

      if (gasLimit) {
        transactionResponse = await this.alice.sendTransaction({
          ...transaction,
          gasLimit: gasLimit,
        });
      } else {
        transactionResponse = await this.alice.sendTransaction(transaction);
      }
    } else {
      tokenInBefore = await getBalanceAndApprove(
        this.alice,
    ...
    const tokenInAfter = await hardhat.getBalance(this.alice._address, tokenIn);
    const tokenOutAfter = await hardhat.getBalance(
      this.alice._address,
      tokenOut
    );
  ...
  /**
   * Function to perform a call to this.executeSwap and validate the response
   * @param quote: CurrencyAmount<Currency>
   ...
   */
  private validateExecuteSwap = async (
    swapType: SwapType,
    quote: CurrencyAmount<Currency>,
    tokenIn: Currency,
    tokenOut: Currency,
  ...
  run = async () => {
    describe('alpha router integration', () => {
      ...
      beforeAll(async () => {
        ...
        this.alice = hardhat.providers[0]!.getSigner();
        const aliceAddress = await this.alice.getAddress();
        expect(aliceAddress).toBe(this.alice._address);
        ...
        const aliceEthBalance = await hardhat.provider.getBalance(
          this.alice._address
        );
        ...
        describe(`Mainnet - ${tradeType}`, () => {
          describe(`+ Execute on Hardhat Fork`, () => {
            it('erc20 -> erc20', async () => {
              ...
              await this.validateExecuteSwap(
            ...
            it('erc20 -> erc20 with permit', async () => {
              ...
              const signature = await this.alice._signTypedData(
                domain,
                types,
                values
              );
            ...
            it(`erc20 -> eth`, async () => {
              ...
              await this.validateSwapRoute(quote, quoteGasAdjusted, tradeType);
            ...
            it(`erc20 -> eth large trade`, async () => {
              ...
              const amountInEdgesTotal = _(route)
                ...
                .map((routeWithValidQuote) =>
                  tradeType == TradeType.EXACT_INPUT
                    ? BigNumber.from(
                        routeWithValidQuote.amount.quotient.toString()
                      )
                    : BigNumber.from(
                        routeWithValidQuote.quote.quotient.toString()
                      )
                )
          ...
          if (this.isTenderlyEnvironmentSet) {
            describe(`+ Simulate on Tenderly + Execute on Hardhat fork`, () => {
              ...
              if (this.isTesterPKEnvironmentSet) {
                it('erc20 -> erc20 with permit with tester pk', async () => {
                  ...
                  const { domain, types, values } =
                    AllowanceTransfer.getPermitData(permit, PERMIT2_ADDRESS, 1);

                  const wallet = new Wallet(this.envVars.TESTER_PK!);
        ...
        describe(`exactIn mixedPath routes`, () => {
          describe('+ simulate swap', () => {
            it('BOND -> APE', async () => {
              ...
              const { quote, quoteGasAdjusted, methodParameters, route } =
                swap!;
  ...
  runTestOnChain = async (chain: ChainId) => {
    for (const tradeType of [TradeType.EXACT_INPUT, TradeType.EXACT_OUTPUT]) {
  ...
  runExternalTests = async () => {
    describe('external class tests', () => {
      ...
      it('Prevents incorrect routes array configurations', async () => {
        ...
        /// Should fail
        await expect(
          onChainQuoteProvider.getQuotesManyExactIn(
            amountIns,
            routes_v3_v2_mixed
          )
        ).rejects.toThrow();
        ...
        await onChainQuoteProvider.getQuotesManyExactIn(amountIns, [
          mixedRoute,
        ]);
