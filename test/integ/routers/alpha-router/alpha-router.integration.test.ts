...
} from '@nizaglobal/universal-router-sdk';
import { Permit2Permit } from '@nizaglobal/universal-router-sdk/dist/utils/inputTokens';
                ...
                const responses = await Promise.all(
                  enableFeeOnTransferFeeFetching.map(
                    async (enableFeeOnTransferFeeFetching) => {
                      ...
                      if (
                        enableFeeOnTransferFeeFetching &&
                        tokenOut?.address === BULLET_WITHOUT_TAX.address
                      ) {
...
describe('quote for other networks', () => {
  const TEST_ERC20_1: { [chainId in ChainId]: () => Token } = {
    ...
    [ChainId.NIZA_TESTNET]: () => USDC_ON(ChainId.NIZA_TESTNET),
    [ChainId.NIZA_LIVENET]: () => USDC_ON(ChainId.NIZA_LIVENET),
  };
  const TEST_ERC20_2: { [chainId in ChainId]: () => Token } = {
    ...
    [ChainId.NIZA_TESTNET]: () => WNATIVE_ON(ChainId.NIZA_TESTNET),
    [ChainId.NIZA_LIVENET]: () => WNATIVE_ON(ChainId.NIZA_LIVENET),
  };
      ...
      describe(`${ID_TO_NETWORK_NAME(chain)} ${tradeType} 2xx`, function () {
        ...
        describe(`Swap`, function () {
          ...
          it(`erc20 -> erc20`, async () => {
            ...
            const exactOutAmount =
              chain === ChainId.BLAST || chain === ChainId.ZORA ? '0.002' : '1';
            const amount =
              tradeType == TradeType.EXACT_INPUT
                ? parseAmount('1', tokenIn)
