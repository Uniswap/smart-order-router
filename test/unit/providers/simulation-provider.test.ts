...
describe('Fallback Tenderly simulator', () => {
  ...
  beforeEach(() => {
    ...
    simulator = new FallbackTenderlySimulator(chainId, provider, tenderlySimulator, ethEstimateGasSimulator);
  });
...
describe('Eth estimate gas simulator', () => {
  ...
  beforeEach(() => {
    simulator = new EthEstimateGasSimulator(chainId, provider, v2PoolProvider, v3PoolProvider);
    permit2Contract = {
      allowance: async () => {
        return {
          amount: BigNumber.from(1000000000),
  ...
  test('simulates when user has sufficient balance and currency is native', async () => {
    ...
    sinon.stub(provider, <any>'getBalance').resolves(BigNumber.from(1000000000))
    const swapRouteWithGasEstimate = await simulator.simulate(
        fromAddress,
        swapOptions,
        { ...swaproute, trade: { tradeType: TradeType.EXACT_INPUT, inputAmount: CurrencyAmount.fromRawAmount(nativeOnChain(1), 0)} as Trade<any, any, any>},
