...
export class V4PoolProvider extends PoolProvider<Currency, V4PoolConstruct, V4ISlot0, V4ILiquidity, V4PoolAccessor> implements IV4PoolProvider {
  ...
  protected instantiatePool(pool: V4PoolConstruct, slot0: V4ISlot0, liquidity: V4ILiquidity): Pool {
    ...
    return new Pool(currency0, currency1, fee, tickSpacing, hooks, slot0.sqrtPriceX96.toString(), liquidity.liquidity.toString(), slot0.tick);
  }
  ...
  protected instantiatePoolAccessor(poolIdentifierToPool: {
    ...
    return {
      getPool: (
        ...
        const { poolIdentifier } = this.getPoolIdentifier([currencyA, currencyB, fee, tickSpacing, hooks]);
        return poolIdentifierToPool[poolIdentifier];
      },
      getPoolById: (poolId: string): Pool | undefined => poolIdentifierToPool[poolId],
