...
describe('SubgraphProvider V2', () => {
  ...
  function constructPool(
    withVirtualToken: boolean,
    trackedReservedEth: string
  ) {
    return {
      id: '0xAddress1',
      token0: {
  ...
  it('fetches subgraph pools if trackedReserveETH is above 0.025 threshold on Base', async () => {
    ...
    subgraphProviderBase = new V2SubgraphProvider(
      ChainId.BASE,
      2,
      30000,
      true,
  ...
  it('fetches subgraph pools if trackedReserveETH is above 0.025 threshold on Mainnet', async () => {
    ...
    subgraphProviderMainnet = new V2SubgraphProvider(
      ChainId.MAINNET,
      2,
      30000,
      true,
  ...
  it('isVirtualPairBaseV2Pool tests', async () => {
    ...
    const mainnetSubgraphProviderV2 = new V2SubgraphProvider(
      ChainId.MAINNET,
      2,
      30000,
      true,
    ...
    expect(
      mainnetSubgraphProviderV2.isVirtualPairBaseV2Pool(
        constructPool(true, '1')
      )
    ).toBe(false);
    expect(
      mainnetSubgraphProviderV2.isVirtualPairBaseV2Pool(
    ...
    const baseSubgraphProviderV2 = new V2SubgraphProvider(
      ChainId.BASE,
      2,
      30000,
      true,
    ...
    expect(
      baseSubgraphProviderV2.isVirtualPairBaseV2Pool(constructPool(true, '1'))
    ).toBe(true);
    expect(
      baseSubgraphProviderV2.isVirtualPairBaseV2Pool(constructPool(false, '1'))
    ).toBe(false);
  });
});
